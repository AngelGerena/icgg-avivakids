...
export const TeacherPortal = () => {
  ...
  const handleCheckout = async (child: any) => {
    ...
    try {
      ...
      let relationship = '';
      if (checkoutForm.pickedUpBy === parent?.primary_name) relationship = parent?.primary_relationship || 'Padre/Madre';
      else if (checkoutForm.pickedUpBy === parent?.secondary_name) relationship = parent?.secondary_relationship || 'Contacto Secundario';
      else if (checkoutForm.pickedUpBy === parent?.approved_pickup_name) relationship = 'Persona Autorizada';
